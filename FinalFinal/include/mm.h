...
// -> GETVAL(x,PAGING_SWPFPN_MASK,PAGING_PTE_SWPOFF_LOBIT)
...
#define PAGING_MAX_PGN  (DIV_ROUND_UP(BIT(PAGING_CPU_BUS_WIDTH),PAGING_PAGESZ)) //thêm chữ BIT vô
...
//Thêm /* PTE BIT SWAPPED */ -> không được đâu :))
//#define PAGING_PTE_SET_IN_SWAP(pte) (pte=pte|PAGING_PTE_SWAPPED_MASK)
#define PAGING_PAGE_IN_SWAP(pte) ((pte & ( (1U << 31) >> 1)) & PAGING_PTE_SWAPPED_MASK)
...
/* Frame PHY Num */
#define PAGING_ADDR_FPN_LOBIT NBITS(PAGING_PAGESZ) // sua thanh -> #define PAGING_ADDR_FPN_LOBIT 0
...
#define PAGING_PTE_LAST13_MASK  GENMASK(12, 0)
...
#define PAGING_FPN(x)  GETVAL(x,PAGING_PTE_LAST13_MASK, 0) 
/* Extract SWAPFPN */
//#define PAGING_PGN(x)  GETVAL(x,PAGING_PGN_MASK,PAGING_ADDR_PGN_LOBIT)
#define PAGING_SWAPFPN(x)  GETVAL(x,PAGING_SWPFPN_MASK,PAGING_PTE_SWPOFF_LOBIT) 
#define PAGING_EXTRACT_LAST13(x)  GETVAL(x,PAGING_PTE_LAST13_MASK,0) 

/* Extract SWAPTYPE */ // không cần đâu, có 1 type mà 
//#define PAGING_FPN(x)  GETVAL(x,PAGING_FPN_MASK,PAGING_ADDR_FPN_LOBIT)
...
int enlist_pgn_node(struct pgn_t **pgnlist, int pgn, struct mm_struct* owner);
...
int find_victim_page(struct mm_struct* mm, int *pgn, uint32_t** ret_ptbl); // -> Thêm uint32_t* ret_ptbl
